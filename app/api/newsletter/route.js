...
export async function POST(request) {
  try {
    ...
    const userMailOptions = {
      from: `${process.env.EMAIL_FROM || 'noreply@collabhub.com'}`,
      to: email,
      subject: 'Welcome to CollabHub Newsletter!',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
          <div style="text-align: center; margin-bottom: 20px;">
            <h1 style="color: #6366f1;">Welcome to CollabHub Newsletter!</h1>
          ...
          <div style="margin: 30px 0; text-align: center;">
            <a href="https://collabhub-1.vercel.app/" style="background: linear-gradient(to right, #6366f1, #a855f7); color: white; text-decoration: none; padding: 12px 24px; border-radius: 6px; font-weight: bold; display: inline-block;">Visit CollabHub</a>
          </div>
    ...
    const adminMailOptions = {
      from: `${process.env.EMAIL_FROM || 'noreply@collabhub.com'}`,
      to: process.env.ADMIN_EMAIL || 'admin@collabhub.com',
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'Subscription successful! Check your email for confirmation. Now please sign in to your account or just create one.' 
    }), { 
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    return new Response(JSON.stringify({ 
      success: false, 
      message: 'Failed to process subscription. Please try again later. You can give a try after signing in.' 
    }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
