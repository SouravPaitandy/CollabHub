...
const NotFound = () => {
  ...
  const [showConnectivityAnimation, setShowConnectivityAnimation] =
    useState(false);
  ...
  return (
    <div
      className={`min-h-screen flex flex-col items-center justify-center px-4 transition-colors duration-300 relative overflow-hidden ${baseClasses}`}
    >
      ...
      {floatingElements.map((el) => (
        <div
          ...
          style={{
            ...
            animation: animateFigures
              ? `float ${el.animationDuration} ease-in-out infinite alternate`
              : "none",
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
          <div
            className={`absolute bg-purple-200 dark:bg-purple-500 opacity-20 rounded-full w-32 h-32 -top-16 -left-16 blur-xl transform -translate-x-full ${animateNumber ? "translate-x-0" : ""} transition-transform duration-1000 ease-out`}
          ></div>
          <div
            className={`absolute bg-blue-200 dark:bg-blue-500 opacity-20 rounded-full w-40 h-40 -bottom-20 -right-20 blur-xl transform translate-x-full ${animateNumber ? "translate-x-0" : ""} transition-transform duration-1000 ease-out`}
          ></div>

          {/* Additional decorative circles */}
          <div
            className={`absolute rounded-full w-12 h-12 top-0 left-1/2 transform -translate-x-1/2 -translate-y-full bg-purple-300 dark:bg-purple-600 opacity-20 blur-md ${
              animateNumber
                ? "animate-[float-subtle_8s_ease-in-out_infinite]"
                : ""
            }`}
          ></div>
          <div
            className={`absolute rounded-full w-6 h-6 bottom-0 left-1/4 transform -translate-y-full bg-blue-300 dark:bg-blue-600 opacity-20 blur-md ${
              animateNumber
                ? "animate-[float-subtle_6s_ease-in-out_infinite_1s]"
                : ""
            }`}
          ></div>
          <div
            className={`absolute rounded-full w-8 h-8 bottom-0 right-1/4 transform -translate-y-full bg-purple-300 dark:bg-purple-600 opacity-20 blur-md ${
              animateNumber
                ? "animate-[float-subtle_7s_ease-in-out_infinite_0.5s]"
                : ""
            }`}
          ></div>
        </div>
