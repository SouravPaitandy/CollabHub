...
const GitHubRepos = ({ onCreateCollab }) => {
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      ...
      try {
        console.log(
          "Fetching GitHub repos with session provider:",
          session.provider
        );
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          >
            ...
            <span className="hidden sm:inline ml-2 mr-2">of </span> {repos.length}{" "}
            <span className="hidden sm:inline ml-2">repositories</span>
          </motion.div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto custom-scrollbar max-h-[550px] p-4">
          <AnimatePresence>
            {repos.slice(0, 8).map((repo, index) => (
              <motion.div
                key={repo.id}
                className={`group bg-gradient-to-br from-white to-gray-50 dark:from-gray-700 dark:to-gray-800 rounded-xl p-6 shadow-sm hover:shadow-xl 
                  border border-transparent hover:border-indigo-300 dark:hover:border-indigo-700 transition-all duration-300
                  ${
                    hoveredRepo === repo.id
                      ? "ring-2 ring-indigo-400 dark:ring-indigo-600 shadow-lg"
                      : ""
                  }`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <h4 className="text-lg font-bold text-gray-800 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {repo.name}
                  </h4>
                ...
                <motion.button
                  onClick={(e) =>{
                    ...
                    const confirmed = confirm(
                      "Are you sre, you want to create a collaboration with this GitHub repo?"
                    );
                    if (confirmed) {
                      onCreateCollab(repo);
                    }
                  }}
                  className="w-full text-sm font-medium px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-lg shadow-sm hover:shadow-indigo-500/30 transition-all flex items-center justify-center"
      ...
      <motion.div
        className="mt-6 p-5 bg-blue-50 dark:bg-blue-900/20 rounded-xl shadow-sm border border-blue-100 dark:border-blue-800/50"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.6 }}
      >
        <div className="flex items-start">
          <div className="flex-shrink-0 mr-3 bg-blue-100 dark:bg-blue-800/50 p-2 rounded-full text-blue-500 dark:text-blue-400">
            <svg
        ...
        <AnimatePresence>
          {selectedRepo && (
            ...
            >
              ...
              >
                <h3 className="text-2xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-purple-600">
                  Import Repository
                </h3>
