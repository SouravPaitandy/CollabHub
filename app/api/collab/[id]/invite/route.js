...
export async function POST(request, { params }) {
    try {
        ...
        if (!participant) {
            return NextResponse.json({ error: "You don't have permission to invite users to this collaboration" }, { status: 403 });
        }
        ...
        if (invitedUser) {
            ...
            if (existingParticipant) {
                return NextResponse.json({ error: "User is already in this collaboration" }, { status: 409 });
            }
        }
        ...
        await transporter.sendMail({
            ...
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 5px;">
                    ...
                    <p>Hello,</p>
                    <p>${currentUser.name || 'Someone'} has invited you to join <strong>${collab.name}</strong> on CollabHub.</p>
                    ...
                    <p style="color: #666; font-size: 14px;">If you don't have a CollabHub account yet, you'll need to sign up first.</p>
                    <hr style="margin: 20px 0; border: none; border-top: 1px solid #e1e1e1;">
                    <p style="color: #999; font-size: 12px;">If you didn't expect this invitation, you can safely ignore this email.</p>
                </div>
            `,
            text: `
                Join "${collab.name}" on CollabHub
                
                Hello,
                
                ${currentUser.name || 'Someone'} has invited you to join "${collab.name}" on CollabHub.
                
                Invite Code: ${collab.inviteCode}
                
                Join here: ${inviteLink}
                
                If you don't have a CollabHub account yet, you'll need to sign up first.
